...
def potencia_deslocada(A, sigma, tol=1e-7, max_iter=200):
    """Encontra o autovalor mais próximo de 'sigma'."""
    n = len(A)
    ...
    for iteracao in range(1, max_iter + 1):
        try:
            y = resolver_sistema_gauss(B, x)
        except ValueError:
            return sigma, x, iteracao

        mu_new = max_abs_vetor(y)
        x = [elem / mu_new for elem in y]

        if abs(mu_new - mu_old) / abs(mu_new) < tol:
            lambda_real = sigma + (1.0 / mu_new)
            return lambda_real, x, iteracao
        
        mu_old = mu_new

    return sigma + (1.0 / mu_new), x, max_iter